...
class syntax_plugin_photogallery extends DokuWiki_Syntax_Plugin {
    ...
    function _findimages(&$data){
        ...
        if ($data['sort'] == 'random') {
            ...
            // reverse?
            if (!empty($data['reverse'])) $files = array_reverse($files);
        }
     ...
     */
    protected function _description($files,$data,$R){
        $imgcnt = 0;
        $vidcnt = 0;
    ...
    protected function _image(&$img,$data,$idx){
        ...
        if (!empty($img['isvid'])){
            ...
            if (($mh > $mw) and ($data['vprot'])) // Invert viewport for portrait images
                ...
                if (!empty($img['fullsize'])) {  // Override image size for fullsize
                        $iw = $mw;
                        $ih = $mh;
                } else {
                        if (!empty($data['crop'])) {
    ...
    function _caption($img,$data){
        ...
        if (!empty($data['showdescription'])) {
            $desc = $this->_meta($img,'desc');
            if(!empty($desc)){
                $ret .= '<p>'.nl2br(hsc($desc)).'</p>';
            }
        }
        if (!empty($data['showkeywords'])) {
     ...
     */
    protected function _auth_check($data){
        global $USERINFO;
        global $INPUT;
        global $auth;
        global $conf;
