...
$PHPTHUMB_CONFIG['high_security_password']      = 'Hgtuigh&546Guyh!*,ghuytjuYYGFfgh';      // required if 'high_security_enabled' is true, and must be at complex (uppercase, lowercase, numbers, punctuation, etc -- punctuation is strongest, lowercase is weakest; see PasswordStrength() in phpthumb.functions.php). You can use a password generator like http://silisoftware.com/tools/password-random.php to generate a strong password
...
function pgThumbURL($ParameterString, $path_to_phpThumb='phpThumb.php') {
	...
	$ParameterString = implode($PHPTHUMB_CONFIG['high_security_url_separator'], $ParamterStringEncodedArray);
	return $path_to_phpThumb.'?'.$ParameterString;
	//return $path_to_phpThumb.'?'.$ParameterString.$PHPTHUMB_CONFIG['high_security_url_separator'].'hash='.md5($ParameterString.$PHPTHUMB_CONFIG['high_security_password']);
}
